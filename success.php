...
if (!isset($_GET['res_id']) || !isset($_GET['external_id'])) {
    die("❌ Missing res_id or external_id");
}

$res_id = mysqli_real_escape_string($conn, $_GET['res_id']);
$external_id = mysqli_real_escape_string($conn, $_GET['external_id']);

// ----------------------------
// 1️⃣ Fetch invoice from Xendit API
// ----------------------------
...
echo "<p>✅ Invoice Status: $invoice_status</p>";

// ----------------------------
// 2️⃣ Update invoice table
// ----------------------------
$update_invoice = "
...
if (!mysqli_query($conn, $update_invoice)) {
    echo "❌ Invoice Update Error: " . mysqli_error($conn);
} else {
    echo "✅ Invoice table updated successfully.<br>";
}

// ----------------------------
// 3️⃣ Fetch reservation
// ----------------------------
$query = "
    SELECT r.*, g.firstname, g.lastname, g.address, g.contactno, g.email, p.payment_method, p.payment_reference, p.payment_status, p.receipt_file, rm.room_number, rm.room_type, rm.room_price, c.cottage_type, c.cottage_price, xi.invoice_id, xi.external_id, xi.status AS invoice_status, xi.amount AS invoice_amount, xi.invoice_url, xi.expiry_date, xi.created_at AS invoice_created FROM reservation r JOIN guest g ON r.guest_id = g.guest_id LEFT JOIN payment p ON r.reservation_id = p.reservation_id LEFT JOIN room rm ON r.room_id = rm.room_id LEFT JOIN cottage c ON r.cottage_id = c.cottage_id LEFT JOIN xendit_invoices xi ON r.reservation_id = xi.reservation_id WHERE r.reservation_id = '$res_id' OR xi.external_id = '$external_id' LIMIT 1
";
...
// ----------------------------
// 4️⃣ Update reservation if paid
// ----------------------------
if ($invoice_status === 'PAID') {
    ...
    if (!mysqli_query($conn, $update)) {
        echo "❌ Reservation Update Error: " . mysqli_error($conn);
    } else {
        echo "✅ Reservation marked as confirmed.<br>";
    }
} else {
    echo "⚠️ Invoice not marked as PAID (Current status: $invoice_status)<br>";
}
